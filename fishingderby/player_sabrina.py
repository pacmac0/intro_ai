...
class PlayerControllerHMM(PlayerControllerHMMAbstract):
    ...
    def guess(self, step, observations):
        ...
        if step < 115:
            # randomly pick fish to guess in the beginning to avoid picking same type over again and collect information of different types
        ...
        # each model represents a fish_type and we calculate P(Obs| Mod) and choose the model with Max(prob of given sequence) for each fish(sequence)
        probabilities = []
    ...
    def reveal(self, correct, fish_id, true_type):
        ...
        # print(known_fish)
        # Update the model which represents the observed true fish type (model gets recomputed each time, because of convergence problem)
        models[true_type] = self.baumWelch(sum([obs_sequences[i] for i in known_fish[true_type]], []), self.initABPi())

        # keep track of when the model of true_type(model) where updated (in time step t)
        global last_train
        ...
        # retrain x other models durring this run of reveal 
        n_retrain = 6
        if t_guessed_before:
            for type_ in [last_train.index(t) for t in sorted(t_guessed_before)[:min(n_retrain, len(t_guessed_before))]]: # choose the model, that had the longest time no update, and retrain
